...
PAGE_URL = f"{BASE_URL}/wiki/Event"
HEADERS = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
}
REQUEST_TIMEOUT = 25
OUTPUT_FILENAME = "wuthering_waves_current_events.json"
...
def clean_image_url(url):
    ...
    # This function is kept in case image URLs are needed elsewhere, but not for the final rewards string format
...
# --- *** UPDATED FUNCTION: Scrape Event Rewards (String Output) *** ---
def scrape_event_rewards(detail_url):
    """
    Visits an event detail page and scrapes rewards, returning a list
    ...
    rewards_list_of_strings = [] # Changed variable name for clarity
    try:
        ...
        possible_header_tags = ['h2', 'h3']
        found_span = soup.find('span', id='Total_Rewards')
        if found_span:
            parent_header = found_span.find_parent(possible_header_tags)
            if parent_header: rewards_header = parent_header; header_found_text = safe_get_text(found_span, "Total Rewards")
        if not rewards_header: # Fallback Text Match
             ...
             for header in all_page_headers:
                  ...
                  if header_text.lower() == 'total rewards': rewards_header = header; header_found_text = header_text; break
        ...
        else: print(f"    - Could not find 'Total Rewards' header. Will search entire page.")

        # --- 2. Find Reward Items ---
        reward_items_found = []; item_selector = 'div.card-container'
        if rewards_header:
        ...
        for item in reward_items_found:
            ...
            # Extract Name
            name_el = item.select_one('span.card-caption a')
            ...
            # Extract Quantity
            quantity_el = item.select_one('span.card-text'); quantity_text = safe_get_text(quantity_el, None)
            final_reward_quantity = parse_quantity(quantity_text)

            # *** CHANGE: Construct and append the string instead of a dictionary ***
            if final_reward_name != "Unknown Reward":
                ...
                rewards_list_of_strings.append(reward_string)
                 ...
                 # Log skipped items only if name was missing
                 print(f"      - Skipping item element, missing name. Content: {' '.join(item.stripped_strings)[:60]}...")
            # *** END CHANGE ***
    ...
    return rewards_list_of_strings # Return the list of strings


# --- Parsing Logic for Main Event Table --- (No changes needed, handles returned list)
def parse_event_table(table_element):
    """Parses the main event table rows AND triggers reward fetching for each event."""
    ...
    for i, row in enumerate(rows):
        ...
        try:
            ...
            event_data['image_url'] = clean_image_url(make_absolute_url(raw_image_url)) # Kept image URL for main event
            ...
            # Fetch Rewards (now returns list of strings)
...
# --- Main Scraping Function --- (No changes needed here)
def scrape_wuthering_waves_events(url):
    """
    Main function to scrape the Wuthering Waves event page for current events
    ...
    try:
        ...
        if not relevant_header:
            for header_id in possible_ids_main:
                 ...
                 if found_span:
                      parent_header = found_span.find_parent(possible_header_tags)
                      if parent_header: relevant_header = parent_header; header_found_method=f"Method 2: <{parent_header.name}> containing span#{header_id}"; break
        # Method 3
        if not relevant_header:
        ...
        while next_element:
            ...
            if isinstance(next_element, Tag):
                ...
                if next_element.name == 'table' and 'article-table' in next_element.get('class',[]) and 'sortable' in next_element.get('class',[]):
                    print(f"  Found target table after skipping {elements_skipped} elements."); target_table = next_element; break
                elements_skipped += 1
            next_element = next_element.find_next_sibling()
        # --- Process Table ---
        if target_table: current_events = parse_event_table(target_table)
        else: print("Main event table not found. Fallback div parsing not active.");
        if not current_events: print("Warning: No events were successfully extracted.")
...
if __name__ == "__main__":
    print("-" * 30); print("Wuthering Waves Fandom Event Scraper"); print("-" * 30)
    start_time = time.time()
    events = scrape_wuthering_waves_events(PAGE_URL)
